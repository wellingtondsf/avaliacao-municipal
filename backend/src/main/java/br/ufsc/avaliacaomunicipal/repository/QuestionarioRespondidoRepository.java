...
public interface QuestionarioRespondidoRepository extends JpaRepository<QuestionarioRespondido, Long> {
	...
	@Query(value = "select * from tb_questionario_respondido respondido where respondido.nu_cpf = ?1 and respondido.co_municipio = ?2 and respondido.co_questionario = ?3", nativeQuery = true)
	QuestionarioRespondido findByCpfMunicipioDate(String cpf, Long municipioId, Long questionarioId, Date data);
